...
const MeetPage = () => {
  ...
  const cleanupMeetStatus = () => {
    ...
    httpClient
      ...
      .catch((err) => {
        console.error("Error cleaning up meet status:", err);
      });
  };

  const handleDocEndMeeting = () => {
    httpClient
      .put("/update_doctor_ratings", {
        demail: searchparams.get("selectedMail"),
        pemail: searchparams.get("pemail"),
        meetLink: window.location.pathname + window.location.search,
        stars: 3,
      })
      .then(() => {
        cleanupMeetStatus();
