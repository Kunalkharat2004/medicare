...
def register():
    ...
    if 'registerer' in request.form:
        ...
        return jsonify({'message': 'Required field "registerer" is missing'}), 400
...
def make_meet():
    ...
    if request.method == 'PUT':
        ...
        doctors.update_one(
            {'email': demail},
            {'$set': {'link': {'link': data['link'], 'name': data['patient']}, 'meet': True}}
        )

        # Set the same meet link for the patient
        patients.update_one(
            {'email': data['pemail']},
            {'$set': {'link': data['link'], 'meet': True}}
        )
    ...
    # Handle POST request: Retrieve meet link
    else:
        user_email = data.get('email')
        # Check if user is a doctor
        doc = doctors.find_one({'email': user_email})
        if doc:
            return jsonify({
                'message': 'Meet link', 
                'link': doc.get('link', None),
                'meet': doc.get('meet', False)
            }), 200
        
        # Check if user is a patient
        pat = patients.find_one({'email': user_email})
        if pat:
            return jsonify({
                'message': 'Meet link', 
                'link': pat.get('link', None),
                'meet': pat.get('meet', False)
            }), 200
...
def meet_status():
    ...
    details = doctors.find_one({'email': user})
        ...
        if details.get('meet', False):
            return jsonify({
                'message': 'Patient is already in a meet', 
                'link': details.get('link', '')
            }), 208
        else:
            if data.get('link', '') == '':
                patients.update_one({'email': user}, {'$set': {'meet': True}})
            else:
                patients.update_one({'email': user}, {'$set': {'meet': True, 'link': data.get('link', '')}})
            return jsonify({'message': 'Patient status updated successfully'}), 200
